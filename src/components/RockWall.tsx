...
export const RockWall = ({parentRef}: { parentRef: React.MutableRefObject<HTMLElement | null> }) => {
    ...
    useEffect(() => {
        ...
        const handleResize = debounce(() => {
            if (target.clientHeight !== parentHeight) {
                console.log("Height Changed to: ", target.clientHeight);
    ...
    const drawRockTexture = (context: CanvasRenderingContext2D, width: number, height: number) => {
        let points = [];
        let x = width / 2;
        let y = height;
        ...
        for (; y > cliffTopStart; y--) {
            let jaggedness = Math.random() * (maxJaggedness - minJaggedness) + minJaggedness;
    ...
    const generateRockyTexture = (context: CanvasRenderingContext2D, width: number, height: number, cliffPoints: Map<number, number>) => {
        const greyPalette = ['#686868', '#707070', '#787878', '#808080', '#888888'];
    ...
    return (
        <Box display={{base: "none", md: "block"}} as={"canvas"} ref={canvasRef} width={200} height={parentHeight}/>
    );
};
