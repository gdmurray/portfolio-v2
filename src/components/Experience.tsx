...
export const ExperienceItem = ({experience, color}: {
    ...
    return (
        <VStack ref={ref} alignItems={"flex-start"} mb={6} id={experience.anchor}>
            ...
            >
                {/*<IconCalendarMonth/>*/}
                {dayjs(experience.dateFrom).format("MMM YYYY")} - {experience.dateTo != null ? dayjs(experience.dateTo).format("MMM YYYY") : "Current"}
            </Heading>
            <VStack alignItems={"flex-start"}>
                {experiences.map((e: any, index: number) => {
                        ...
                        return (
                            ...
                            >
                                <Box w={"32px"} h={"32px"}>
                                    <IconChevronRight width={32} height={32} size={28}
...
export const Experience = (props: Queries.ExperienceComponentFragment) => {
    ...
    return (
        <Section bg={"brand.background.green"} title={"4. Experience"} anchor={"experience"} isLast={true}>
            <Stack maxW={"5xl"}>
